...
int main(int argc, char** argv)
{
   ...
   // Get Accelerator compute device
    ...
    d_a = clCreateBuffer(context,  CL_MEM_READ_WRITE,  sizeof(int) * number_of_words, NULL, NULL);
    ...
    err = clEnqueueWriteBuffer(commands, d_a, CL_TRUE, 0, sizeof(int) * number_of_words, h_a_input, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_a); // Not used in example RTL logic.
    err |= clSetKernelArg(kernel, 2, sizeof(cl_mem), &d_a); // Not used in example RTL logic.
    ...
    for (uint i = 0; i < number_of_words; i++) {
        if ((h_a_input[i] + 1) != h_a_output[i]) {
            printf("ERROR in AdderAxi - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_a_input[i], h_a_input[i], h_a_output[i], h_a_output[i]);
            check_status = 1;
        }
